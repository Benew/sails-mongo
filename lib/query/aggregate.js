...
function addGroupOption(opt, aggregateFnName, group){
  if("object" === typeof opt){
    _.forEach(opt, function(val, key){
      group[key] = group[key] || {};
      group[key][aggregateFnName] = val;
    });
  } else {
    group[opt] = group[opt] || {};
    group[opt][aggregateFnName] = '$' + opt;
  }
}
